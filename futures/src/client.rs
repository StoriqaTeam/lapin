...
fn heartbeat_pulse<T: AsyncRead + AsyncWrite + Send + 'static>(
    ...
    future::select_all(vec![
        future::Either::A(
            rx.map(|_| debug!("Stopping heartbeat"))
                .or_else(|_| future::empty()),
        ),
        future::Either::B(
            future::result(interval)
                .or_else(|_| future::empty())
...
fn make_heartbeat<F, Pulse>(pulse_maker: F) -> Heartbeat<Pulse>
where
    F: FnOnce(oneshot::Receiver<()>) -> Pulse,
{
