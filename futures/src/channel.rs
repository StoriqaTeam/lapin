...
impl<T> Channel<T> {
    pub fn is_connected(&self) -> bool {
        ...
        transport.conn.is_connected(self.id)
    }
}
...
impl<T: AsyncRead + AsyncWrite + Send + Sync + 'static> Channel<T> {
    ...
    pub fn create(
        ...
        future::poll_fn(move || {
            ...
            channel
                .run_on_locked_transport("create", "Could not create channel", move |transport| {
                    transport
                        .conn
                        .channel_open(channel_id, "".to_string())
                        .map(Some)
                }).and_then(move |_| {
                    future::poll_fn(move || {
        ...
        self.run_on_locked_transport(
            ...
            future::poll_fn(move || {
                ...
                if let Some(queue) = transport
                    .conn
                    .get_generated_name(request_id.expect("expected request_id"))
                {
                    let (consumer_count, message_count) = if let Some(async_queue) = transport
                        .conn
                        .channels
                        .get(&channel_id)
                        .and_then(|channel| channel.queues.get(&queue))
                    {
                        (async_queue.consumer_count, async_queue.message_count)
                    } else {
                        (0, 0)
                    };
                    return Ok(Async::Ready(Queue::new(
                        queue,
                        consumer_count,
                        message_count,
                    )));
    ...
    pub fn queue_unbind(
        ...
        self.run_on_locked_transport(
            "queue_unbind",
            "Could not unbind queue from the exchange",
            move |transport| {
                transport
                    .conn
                    .queue_unbind(
    ...
    pub fn confirm_select(
        ...
        self.run_on_locked_transport(
            "confirm_select",
            "Could not activate confirm extension",
            move |transport| {
                transport
                    .conn
                    .confirm_select(channel_id, options.nowait)
                    .map(Some)
            },
        ).map(|_| ())
    }
    ...
    ) -> impl Future<Item = Option<RequestId>, Error = io::Error> + Send + 'static {
        let channel_id = self.id;
        let exchange = exchange.to_string();
        let routing_key = routing_key.to_string();

        self.run_on_locked_transport_full(
            "basic_publish",
            "Could not publish",
            move |transport| {
                transport
                    ...
                    .and_then(|c| {
                        if c.confirm {
                            if c.acked.remove(&delivery_tag) {
                                ...
                                info!(
                                    "message with tag {} still in unacked: {:?}",
                                    delivery_tag, c.unacked
                                );
                                task::current().notify();
    ...
    pub fn basic_consume(
        ...
        arguments: FieldTable,
    ) -> impl Future<Item = Consumer<T>, Error = io::Error> + Send + 'static {
        let channel_id = self.id;
        let transport = self.transport.clone();
        ...
        self.run_on_locked_transport(
            ...
            future::poll_fn(move || {
                let mut transport = lock_transport!(transport);
                if let Some(consumer_tag) = transport
                    .conn
                    .get_generated_name(request_id.expect("expected request_id"))
                {
    ...
    pub fn basic_ack(
        ...
        self.run_on_locked_transport("basic_ack", "Could not ack message", move |transport| {
            transport
                .conn
                .basic_ack(channel_id, delivery_tag, multiple)
                .map(|_| None)
        }).map(|_| ())
    }
    ...
    pub fn basic_nack(
        ...
        self.run_on_locked_transport("basic_nack", "Could not nack message", move |transport| {
            transport
                .conn
                .basic_nack(channel_id, delivery_tag, multiple, requeue)
                .map(|_| None)
        }).map(|_| ())
    }
    ...
    pub fn basic_reject(
        ...
        self.run_on_locked_transport(
            ...
            move |transport| {
                transport
                    .conn
                    .basic_reject(channel_id, delivery_tag, requeue)
                    .map(|_| None)
            },
        ).map(|_| ())
    }
    ...
    pub fn basic_get(
    ...
    ) -> impl Future<Item = BasicGetMessage, Error = io::Error> + Send + 'static {
        let channel_id = self.id;
        ...
        self.run_on_locked_transport_full(
            ...
            move |transport| {
                transport
                    .conn
                    .basic_get(channel_id, options.ticket, queue, options.no_ack)
                    .map(Some)
            },
            |conn, request_id| match conn.finished_get_result(request_id) {
    ...
    pub fn queue_purge(
        ...
        self.run_on_locked_transport("queue_purge", "Could not purge queue", move |transport| {
            transport
                .conn
                .queue_purge(channel_id, options.ticket, queue_name, options.nowait)
                .map(Some)
        }).map(|_| ())
    }
    ...
    pub fn close(
        ...
        self.run_on_locked_transport("close", "Could not close channel", move |transport| {
            transport
                .conn
                .channel_close(channel_id, code, message, 0, 0)
                .map(|_| None)
        }).map(|_| ())
    }
    ...
    pub fn close_ok(&self) -> impl Future<Item = (), Error = io::Error> + Send + 'static {
        ...
        self.run_on_locked_transport(
            "close_ok",
            "Could not ack closed channel",
            move |transport| transport.conn.channel_close_ok(channel_id).map(|_| None),
        ).map(|_| ())
    }
    ...
    pub fn channel_flow(
        ...
        self.run_on_locked_transport(
            "channel_flow",
            "Could not update channel flow",
            move |transport| {
                transport
                    .conn
                    .channel_flow(channel_id, options.active)
                    .map(|_| None)
            },
        ).map(|_| ())
    }
    ...
    pub fn channel_flow_ok(
        ...
        self.run_on_locked_transport(
            "channel_flow_ok",
            "Could not ack update to channel flow",
            move |transport| {
                transport
                    .conn
                    .channel_flow_ok(channel_id, options.active)
                    .map(|_| None)
            },
        ).map(|_| ())
    }

    fn run_on_locked_transport_full<Action, Finished>(
        &self,
        method: &str,
        error: &str,
        action: Action,
        finished: Finished,
        payload: Option<(Vec<u8>, BasicProperties)>,
    ) -> impl Future<Item = Option<RequestId>, Error = io::Error> + Send + 'static
    where
        Action: 'static
            + Send
            + FnOnce(&mut AMQPTransport<T>)
                -> Result<Option<RequestId>, lapin_async::error::Error>,
        Finished:
            'static + Send + Fn(&mut Connection, RequestId) -> Poll<Option<RequestId>, io::Error>,
    {
    ...
    fn run_on_lock_transport_basic_finished(
        conn: &mut Connection,
        request_id: RequestId,
    ) -> Poll<Option<RequestId>, io::Error> {
    ...
    fn run_on_locked_transport<Action>(
        &self,
        method: &str,
        error: &str,
        action: Action,
                ...
                -> Result<Option<RequestId>, lapin_async::error::Error>,
    {
        self.run_on_locked_transport_full(
            ...
            error,
            action,
            Self::run_on_lock_transport_basic_finished,
            None,
        )
    }
            ...
            'static + Send + Fn(&mut Connection, RequestId) -> Poll<Option<RequestId>, io::Error>,
    {
        ...
        tr.poll()?;
        trace!(
            "wait for answer transport poll; request_id={:?} status=NotReady",
            request_id
        );
        if let Async::Ready(r) = finished(&mut tr.conn, request_id)? {
            trace!(
                "wait for answer; request_id={:?} status=Ready result={:?}",
                request_id,
                r
            );
        ...
        trace!(
            "wait for answer; request_id={:?} status=NotReady",
            request_id
        );
        task::current().notify();
        Ok(Async::NotReady)
